...
namespace WSD_Project_C2004V_4.Controllers
{
    ...
    public class TokenController : Controller
    {
        ...
        private async Task<IActionResult> GetToken(TokenRequestViewModel model)
        {
            try
            {
                ...
                var issuerSignKey = new SymmetricSecurityKey(Encoding.UTF8.GetBytes(Configuration["Auth:Jwt:Key"]));
                var token = new JwtSecurityToken(issuer: Configuration["Auth:Jwt:Issuer"], audience: Configuration["Auth:Jwt:Audience"],
                claims: claims, notBefore: now, expires: now.Add(TimeSpan.FromMinutes(tokenExpirationMins)), signingCredentials: new SigningCredentials(issuerSignKey, SecurityAlgorithms.HmacSha256));
